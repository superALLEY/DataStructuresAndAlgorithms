...
char *Removecharacter(char word[])
{
...
void ReverseWords(char *phrase)
{
  ...
  strcpy(sommet(p), Removecharacter(sommet(p)));

  afficherPile(p);
}

void ReverseCharactersAndWords(char *phrase, int i)
{
...
void ReverseCharacters(char *phrase)
{
  ReverseCharactersAndWords(phrase, 1);
}
...
void replaceWords(char phrase[], char *oldWord, char *newWord)
{
  bool trouve = false;
...
void Piles()
{
  ...
  case 3:
    ...
    ReverseCharactersAndWords(phr, 0);
    break;
  case 4:
  ...
  case 5:
    ...
    replaceWords(p, oldWord, newWord);
    break;
...
File *SuppPrio(File *F, int prio)
{
  ...
  while (temp != NULL)
  {
    if (temp->priority == prio)
    {
      ...
      F = supprimerPos(F, temp, temp->priority);
      F = concatene(F, tempo);
    }
    temp = temp->suiv;
  }
...
void SupprimerHautePrio(File *F, int choice)
{
  int pri, size;
  char msg[taille];
